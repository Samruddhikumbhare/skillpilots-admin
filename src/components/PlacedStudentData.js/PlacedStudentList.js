...
const getPlacedStudentDataApi = async () => {
  try {
    const res = await axios.get(api + "/newskill/selectedStatusRound");
...
const PlacedStudentList = ({ searchString }) => {
  ...
  const [loaderPlacedStudent, setPlacedStudentLoader] = useState(false);
  // Handle page and page size change

  useEffect(() => {
    setPlacedStudentLoader(true);
    getPlacedStudentDataApi()
      .then((formattedData) => {
        setData(formattedData);

        const filteredData = formattedData
          .filter((row) => {
            const rowDate = new Date(row.round_date); // Convert row date to Date object
            return (
    ...
    if (searchString == "") {
      ...
      const filterdatalist = filterdata.filter(
        (entry) =>
          ...
          entry?.job_role?.toLowerCase().includes(searchString.toLowerCase())
      );
  ...
  const handleDownload = () => {
    exportToExcel(filterdata, "complete job role.xlsx");
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Typography
          variant="subtitle2"
          color={params.row.status.toLowerCase() === "selected" ? "green" : "red"}
          fontWeight="bold"
          sx={{
            backgroundColor:
              params.row.status.toLowerCase() === "selected" ? "lightgreen" : "#fac3ca",
            borderRadius: "4px",
            display: "flex",
            alignItems: "center",
            p: 1,
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <div>
            {new Date(params.row.round_date).toLocaleString("en-GB", {
              day: "2-digit",
              month: "2-digit",
              year: "numeric",
            })}
  ...
  useEffect(() => {
    if (inputFilter.toDate !== "" && inputFilter.fromDate !== "") {
      const filteredData = data.filter((row) => {
        const rowDate = new Date(`${row.round_date}`); // YYYY-MM-DD format
...
PlacedStudentList.defaultProps = {
  searchString: "",
};
